...
/**
 * Class EachNthAfterMthFixed
 * @package PHPStudios\ExtCartPriceRules\Model\SalesRule\Rule\Action\Discount
 */
class EachNthAfterMthFixed extends EachNth
{
    /**
     * EachNthAfterMthFixed constructor.
    ...
    protected function _calculate($rule, $item)
    {
        ...
        if ($maxQtyDiscounted > 0) {
            ...
            $afterMth = $ruleCustomAttributes['after_mth'];
            $address = $item->getAddress();
            $items = $address->getAllItems();
            $filteredItems = $this->getFilteredItems($items);
            $isItemNthData = $this->isCurrentItemNth($filteredItems , $item , count($filteredItems) , $eachNth , $afterMth);
            if ($isItemNthData['isItemNth'] == true) {
                ...
                $discountOnItem = ($itemPrice >= $fixedDisAmount) ? $fixedDisAmount : $itemPrice;
