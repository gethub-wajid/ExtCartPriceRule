...
/**
 * Class ExpensivePercent
 * @package PHPStudios\ExtCartPriceRules\Model\SalesRule\Rule\Action\Discount
 */
class ExpensivePercent extends AbstractRule
{
    /**
     * ExpensivePercent constructor.
    ...
    private function _calculate($rule , $item)
    {
        ...
        $discountedItem = $this->getMostExpensiveItem($items);
     ...
     * @return mixed
     */
    private function getMostExpensiveItem($items)
    {
        $expensiveItemKey = -1;
        foreach ($items as $key => $item) {
            if ($expensiveItemKey == -1) {
                $expensiveItemKey = $key;
            }
            if ($item->getProduct()->getPrice() > $items[$expensiveItemKey]->getProduct()->getPrice()) {
                $expensiveItemKey = $key;
            }
        }
        return $items[$expensiveItemKey];
    }
}
