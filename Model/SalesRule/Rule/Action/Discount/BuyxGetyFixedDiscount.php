...
/**
 * Class BuyxGetyFixedDiscount
 * @package PHPStudios\ExtCartPriceRules\Model\SalesRule\Rule\Action\Discount
 */
class BuyxGetyFixedDiscount extends BuyxGety
{
    /**
     * BuyxGetyFixedDiscount constructor.
    ...
    protected function _calculate($rule, $item)
    {
        ...
        if (isset($xyProducts['x']) && isset($xyProducts['y'])) {
            ...
            $fixedDisAmount = $rule->getDiscountAmount();
            ...
            if ($xQty >= $numXProducts && $maxQty > 0) {
                ...
                $yQtyUsed = $this->getYProductQtyConsumed($xyProducts['y'] , $item , $fixedDisAmount , $maxDisAmount , $maxQty , $yDiscQty);
                if ($yQtyUsed >= $yDiscQty) {
                    return $discountData;
                }
                $yQtyRemaining = $yDiscQty - $yQtyUsed;
                foreach ($xyProducts['y'] as $yProductItem) {
                    if (($item->getProduct()->getData('type_id') == 'simple') && ($yDiscQty > 0)
                        && ($yProductItem->getProduct()->getId() == $item->getProduct()->getId())) {
                        for ($i=0; $i<$yProductItem->getQty(); $i++) {
                            $discountAmount += ($yProductItem->getPrice() > $fixedDisAmount) ? $fixedDisAmount : $yProductItem->getPrice();
     ...
     * @param $yProducts
     * @param $item
     * @param $fixedDisAmount
     * @param $maxDisAmountPerItem
     * @param $maxQtyPerItem
     * @param $yDiscQty
     * @return int
     */
    private function getYProductQtyConsumed($yProducts , $item , $fixedDisAmount , $maxDisAmountPerItem , $maxQtyPerItem , $yDiscQty)
    {
        $yQtyUsed = 0;
        $discountAmount = 0;
        foreach ($yProducts as $yProductItem) {
            ...
            if ($item->getProduct()->getData('type_id') == 'simple') {
                ...
                for ($i=0; $i<$yProductItem->getQty(); $i++) {
                    $discountAmount += ($yProductItem->getPrice() > $fixedDisAmount) ? $fixedDisAmount : $yProductItem->getPrice();
                    $discOnItem += ($yProductItem->getPrice() > $fixedDisAmount) ? $fixedDisAmount : $yProductItem->getPrice();
